...
export default function AuthedHomePage() {
  ...
  const [showActionsMap, setShowActionsMap] = useState<Record<string, boolean>>({});
              ...
              <div key={w._id} className="group relative overflow-hidden rounded-[2rem] bg-[#161618] p-6 transition-all hover:bg-[#1c1c1e] border border-white/5 hover:border-purple-500/20 hover:shadow-lg hover:shadow-purple-900/10">
                ...
                <div className="mb-4 flex items-start justify-between relative z-10">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                        ...
                        className="hidden sm:inline-flex rounded-full bg-red-500/10 p-3 text-red-400 hover:bg-red-500 hover:text-white transition-colors"
                        title="Delete Wishlist"
                    >
                         <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
                    </button>
                    <Link
                      href={`/wishlist/${w._id}/edit`}
                      className="hidden sm:inline-flex rounded-full bg-zinc-500/10 p-3 text-zinc-400 hover:bg-zinc-500 hover:text-white transition-colors"
                      title="Edit Wishlist"
                    >
                    ...
                    <Link
                      ...
                      className="hidden sm:inline-flex rounded-full bg-purple-500/10 p-3 text-purple-400 hover:bg-purple-500 hover:text-white transition-colors"
                      title="Add items"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setShowActionsMap((prev) => ({ ...prev, [w._id]: !prev[w._id] }))}
                      className="rounded-full bg-purple-500/10 p-3 text-purple-400 group-hover:bg-purple-500 group-hover:text-white transition-colors sm:hidden"
                ...
                {showActionsMap[w._id] && (
                  <div className="sm:hidden flex flex-col gap-2 mb-3">
                    <Link
                      href={`/wishlist/${w._id}/items`}
                      className="rounded-xl bg-purple-500/10 px-4 py-2 text-xs text-purple-400 hover:bg-purple-500/20"
                    >
                      Add items
                    </Link>
                    <Link
                      href={`/wishlist/${w._id}/edit`}
                      className="rounded-xl bg-zinc-500/10 px-4 py-2 text-xs text-zinc-400 hover:bg-zinc-500/20"
                    >
                      Edit
                    </Link>
                    <button
                      type="button"
                      onClick={() => handleDelete(w._id)}
                      className="rounded-xl bg-red-500/10 px-4 py-2 text-xs text-red-400 hover:bg-red-500/20"
                    >
                      Delete
                    </button>
                  </div>
